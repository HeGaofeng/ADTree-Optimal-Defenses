...
public class AddDomainDialog extends JDialog implements ActionListener, ListSelectionListener, Comparator<Domain> {
	...
	public AddDomainDialog(final MainWindow newParent) {
		super(newParent, "增加属性域", true);
		chosenDomain = null;
		selectPressed = false;
		frame = newParent;
		frame.setFont(new Font("宋体", 20, Font.BOLD));
		setAlwaysOnTop(true);
	...
	private void createLayout() {
		...
		setButton = new JButton("增加");
		setButton.setActionCommand("增加");
		...
		JLabel label = new JLabel("增加属性域:");
		label.setLabelFor(list);
		listPane.add(label);
		listPane.add(Box.createRigidArea(new Dimension(0, 5)));
		listPane.add(listScroller);
		listPane.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));
		description = new JLabel("未初始化!") {
		...
		description.setFont(new Font("宋体", Font.TRUETYPE_FONT, 13));
		...
		descPane.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10),
				BorderFactory.createTitledBorder("详细描述:")));
