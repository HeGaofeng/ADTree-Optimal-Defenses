...
public class ADTreeCanvas extends JPanel implements Scrollable, TreeChangeListener, Printable, Pageable {
	...
	protected void paintLabels(final Graphics2D g, final ADTreeNode node, final Color textCol) {   //重中之重
		final Rectangle2D.Double box = bufferedLayout.get(node);
		int x = (int) box.x;
		int y = (int) box.y;
	...
	protected void paintBox(final Graphics2D g, final ADTreeNode node) {
		...
		if (node.getType() == Choices.canv_Defender) {
			borderCol = Choices.canv_BorderColorDef;    //防御节点的边框颜色
			textCol = Choices.canv_TextColorDef;        //防御节点的文本颜色
			shape = Choices.canv_ShapeDef;              //防御节点的形状
		...
		paintLabels(g, node, textCol);      //重中之重
